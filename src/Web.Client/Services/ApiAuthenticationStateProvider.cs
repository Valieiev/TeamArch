...
namespace Web.Client.Services
{
    public class ApiAuthenticationStateProvider : AuthenticationStateProvider
    {
        ...
        private IEnumerable<Claim> ParseClaimsFromJwt(string jwt)
        {
            ...
            keyValues.TryGetValue(ClaimTypes.NameIdentifier, out var key);
            if (key != null)
            {
                if (key.ToString().Trim().StartsWith("["))
                {
                    var parsedkey = JsonSerializer.Deserialize<string[]>(key.ToString());
                    foreach (var parsed in parsedkey) claims.Add(new Claim(ClaimTypes.NameIdentifier, parsed));
                }
            }
            if (roles != null)
            {
