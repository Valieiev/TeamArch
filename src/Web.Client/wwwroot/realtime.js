...
window.ChatClient = {
    ...
    Start: function(key, hubUrl, assembly, method) {
        ...
        connection.on("ReceiveMessage",
            (username, question, session) => {
                ...
                DotNet.invokeMethod(assembly, method, key, "ReceiveMessage", username, question, session);
            });
    ...
    //
    Register: function(key, username, slug) {
        ...
        // send message (async)
        return connection.invoke("Register", username, slug);
    },

    // 
    // function called when Blazor client wishes to send a message via SignalR
    //
    Send: function(key, username, question, session, slug) {
        console.log("Connection send request");
        var connection = connections[key];
        if (!connection) throw "Connection not found for " + key;
        console.log("Connection located");
        // send message (async)
        return connection.invoke("SendMessage", username, question, session, slug);
    },
    //
